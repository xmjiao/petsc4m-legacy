/*
 * sys/codegen/lib/petscBarrier/petscBarrier_mex.c
 *
 * Auxiliary code for mexFunction of petscBarrier
 *
 * C source code generated by m2c.
 * %#m2c options:896646a58082245b32e90f553216db48
 *
 */
...
static void __petscBarrier_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       obj;
    int32_T             *errCode;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("petscBarrier:WrongInputType",
            "Input argument obj has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscBarrier:WrongSizeOfInputArg",
            "Argument obj should be a scalar.");
    marshallin_const_M2C_OpaqueType(&obj, prhs[0], "obj");
    ...
    petscBarrier(&obj, errCode, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&obj);
    plhs[0] = move_scalar_to_mxArray(errCode, mxINT32_CLASS);
    plhs[1] = move_scalar_to_mxArray(toplevel, mxLOGICAL_CLASS);

}

static void __petscBarrier0_api(mxArray **plhs, const mxArray ** prhs) {
    int32_T             *errCode;
    boolean_T           *topLevel;
    ...
    /* Invoke the target function */
    petscBarrier0(errCode, topLevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscBarrier:TooManyOutputArguments",
                "Too many output arguments for entry-point petscBarrier.\n");
        /* Call the API function. */
        __petscBarrier_api(outputs, prhs);
    }
    else if (nrhs == 0) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscBarrier0:TooManyOutputArguments",
                "Too many output arguments for entry-point petscBarrier0.\n");
        /* Call the API function. */
        __petscBarrier0_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscBarrier:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscBarrier.");
