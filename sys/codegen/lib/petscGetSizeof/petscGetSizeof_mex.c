...
void petscGetSizeof_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscGetSizeof_initialize();

    petscGetSizeof(&name, sz, toplevel);

    petscGetSizeof_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscGetSizeof:TooManyOutputArguments","Too many output arguments for entry-point petscGetSizeof.");
        /* Call the API function. */
        petscGetSizeof_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscGetSizeof:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscGetSizeof.");
