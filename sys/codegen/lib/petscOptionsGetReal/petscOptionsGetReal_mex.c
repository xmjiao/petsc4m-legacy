...
void petscOptionsGetReal_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscOptionsGetReal:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscOptionsGetReal:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[3], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscOptionsGetReal_initialize();

    petscOptionsGetReal(&opts, &pre, &name, value, found, errCode, toplevel);

    petscOptionsGetReal_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 4)
            mexErrMsgIdAndTxt("petscOptionsGetReal:TooManyOutputArguments","Too many output arguments for entry-point petscOptionsGetReal.");
        /* Call the API function. */
        petscOptionsGetReal_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscOptionsGetReal:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscOptionsGetReal.");
