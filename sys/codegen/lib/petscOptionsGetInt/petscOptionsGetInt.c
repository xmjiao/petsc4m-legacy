...
void petscOptionsGetInt(const struct0_T *opts, const emxArray_char_T *pre, const
  emxArray_char_T *name, int *ivalue, int *found, int *errCode, boolean_T
  *toplevel)
{
  PetscBool b_flag;
  boolean_T p;
  ...
  *errCode = PetscOptionsGetInt(c_opts, &pre->data[0], &name->data[0], ivalue,
    &b_flag);
  *found = (int)(b_flag);
