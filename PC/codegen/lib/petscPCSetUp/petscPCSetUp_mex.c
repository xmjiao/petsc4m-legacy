/*
 * PC/codegen/lib/petscPCSetUp/petscPCSetUp_mex.c
 *
 * Auxiliary code for mexFunction of petscPCSetUp
 *
...
void petscPCSetUp_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscPCSetUp_initialize();

    petscPCSetUp(&pc, errCode, toplevel);

    petscPCSetUp_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscPCSetUp:TooManyOutputArguments","Too many output arguments for entry-point petscPCSetUp.");
        /* Call the API function. */
        petscPCSetUp_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscPCSetUp:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscPCSetUp.");
