...
static void __petscPCSetUp_api(mxArray **plhs, const mxArray ** prhs) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscPCSetUp:TooManyOutputArguments",
                "Too many output arguments for entry-point petscPCSetUp.\n");
        /* Call the API function. */
        __petscPCSetUp_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscPCSetUp:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscPCSetUp.");
