...
static void __petscPCFactorSetUpMatSolverPackage_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            pc;
    int32_T             *errCode;
    boolean_T           *toplevel;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscPCFactorSetUpMatSolverPackage:TooManyOutputArguments",
                "Too many output arguments for entry-point petscPCFactorSetUpMatSolverPackage.\n");
        /* Call the API function. */
        __petscPCFactorSetUpMatSolverPackage_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscPCFactorSetUpMatSolverPackage:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscPCFactorSetUpMatSolverPackage.");
