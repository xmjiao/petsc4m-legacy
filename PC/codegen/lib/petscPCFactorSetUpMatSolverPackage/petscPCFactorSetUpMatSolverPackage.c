...
static void b_m2c_error(int varargin_3)
{
  ...
  fmt = "PCFactorSetUpMatSolverPackage returned error code %d\n";
...
void petscPCFactorSetUpMatSolverPackage(const struct0_T *pc, int *errCode,
  boolean_T *toplevel)
{
  boolean_T p;
  boolean_T b_p;
  ...
  t_pc = *(PC*)(&data->data[0]);
  *errCode = PCFactorSetUpMatSolverPackage(t_pc);
  *toplevel = true;
...
void petscPCFactorSetUpMatSolverPackage_initialize(void)
{
}

void petscPCFactorSetUpMatSolverPackage_terminate(void)
{
}
