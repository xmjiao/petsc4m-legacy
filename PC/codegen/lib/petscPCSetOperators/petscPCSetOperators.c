  ...
  struct0_T *Pmat, int *errCode, boolean_T *toplevel)
{
  ...
  if (!p) {
    ...
    for (k = 0; k < loop_ub; k++) {
      b_pc->data[b_pc->size[0] * k] = Amat->type->data[Amat->type->size[0] * k];
    }

    b_pc->data[b_pc->size[0] * Amat->type->size[1]] = '\x00';
    b_m2c_error(b_pc);
  }
  ...
  if (!p) {
    ...
    for (k = 0; k < loop_ub; k++) {
      b_pc->data[b_pc->size[0] * k] = Pmat->type->data[Pmat->type->size[0] * k];
    }

    b_pc->data[b_pc->size[0] * Pmat->type->size[1]] = '\x00';
    b_m2c_error(b_pc);
  }
