...
void petscPCApplyTranspose(const struct0_T *pc, const struct0_T *b, const
  struct0_T *x, int *errCode, boolean_T *toplevel)
{
  ...
  if (!p) {
    ...
    for (k = 0; k < loop_ub; k++) {
      b_pc->data[b_pc->size[0] * k] = b->type->data[b->type->size[0] * k];
    }

    b_pc->data[b_pc->size[0] * b->type->size[1]] = '\x00';
    b_m2c_error(b_pc);
  }
  ...
  if (!p) {
    ...
    for (k = 0; k < loop_ub; k++) {
      b_pc->data[b_pc->size[0] * k] = x->type->data[x->type->size[0] * k];
    }

    b_pc->data[b_pc->size[0] * x->type->size[1]] = '\x00';
    b_m2c_error(b_pc);
  }
...
void petscPCApplyTranspose_2args(const struct0_T *ksp, const struct0_T *b, int
