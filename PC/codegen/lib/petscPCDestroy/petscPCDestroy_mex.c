 ...
 * %#m2c options: codegenArgs=-args {PetscPC}  enableInline=1
 *
 */
...
void petscPCDestroy_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(plhs[0])!=3)
        mexErrMsgIdAndTxt("petscPCDestroy:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscPCDestroy:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscPCDestroy_initialize();

    petscPCDestroy(&pc, errCode, toplevel);

    petscPCDestroy_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscPCDestroy:TooManyOutputArguments","Too many output arguments for entry-point petscPCDestroy.");
        /* Call the API function. */
        petscPCDestroy_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscPCDestroy:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscPCDestroy.");
