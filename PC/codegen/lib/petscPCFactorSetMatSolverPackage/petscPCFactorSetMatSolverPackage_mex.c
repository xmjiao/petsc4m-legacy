/*
 * PC/codegen/lib/petscPCFactorSetMatSolverPackage/petscPCFactorSetMatSolverPackage_mex.c
 *
 * Auxiliary code for mexFunction of petscPCFactorSetMatSolverPackage
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {PetscPC, PetscMatSolverPackage} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscPCFactorSetMatSolverPackage_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscPCFactorSetMatSolverPackage:WrongInputType",
            "Input argument pc has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscPCFactorSetMatSolverPackage:InputStructWrongFields",
            "Input argument pc has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscPCFactorSetMatSolverPackage:WrongSizeOfInputArg",
            "Argument pc must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscPCFactorSetMatSolverPackage:WrongInputStruct",
    ...
    *(void**)&pc.type = mxCalloc(1, sizeof(emxArray__common));
    ...
    alias_mxArray_to_emxArray(prhs[1], (emxArray__common *)&solver, "solver", 2);
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscPCFactorSetMatSolverPackage_initialize();

    petscPCFactorSetMatSolverPackage(&pc, &solver, errCode, toplevel);

    petscPCFactorSetMatSolverPackage_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscPCFactorSetMatSolverPackage:TooManyOutputArguments","Too many output arguments for entry-point petscPCFactorSetMatSolverPackage.");
        /* Call the API function. */
        petscPCFactorSetMatSolverPackage_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscPCFactorSetMatSolverPackage:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscPCFactorSetMatSolverPackage.");
