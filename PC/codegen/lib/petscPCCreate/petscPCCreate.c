                   ...
                   *toplevel)
{
  ...
  *toplevel = true;
  ...
  emxEnsureCapacity((emxArray__common *)pc->data, i0, (int)sizeof(unsigned char));
  k = data0->size[0];
  for (i0 = 0; i0 < k; i0++) {
    pc->data->data[i0] = data0->data[i0];
  }

  emxFree_uint8_T(&data0);
  i0 = pc->type->size[0] * pc->type->size[1];
  pc->type->size[0] = 1;
  pc->type->size[1] = 2;
  emxEnsureCapacity((emxArray__common *)pc->type, i0, (int)sizeof(char));
  for (i0 = 0; i0 < 2; i0++) {
    pc->type->data[i0] = t1_type[i0];
  }
...
void petscPCCreate_World(struct0_T *ksp, int *errCode, boolean_T *toplevel)
{
  ...
  int i;
  emxInit_uint8_T(&data0, 1);
  b_errCode = PCCreate(PETSC_COMM_WORLD, &t_pc);
  sizepe = sizeof(PC);
  i2 = data0->size[0];
  data0->size[0] = sizepe;
