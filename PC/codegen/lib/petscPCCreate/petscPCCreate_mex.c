...
void petscPCCreate_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscPCCreate_initialize();

    petscPCCreate(&comm, &pc, errCode, toplevel);

    petscPCCreate_terminate();
...
void petscPCCreate_World_api(const mxArray ** prhs, const mxArray **plhs) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscPCCreate:TooManyOutputArguments","Too many output arguments for entry-point petscPCCreate.");
        /* Call the API function. */
        petscPCCreate_api(prhs, (const mxArray**)outputs);
    }
    else if (nrhs == 0) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscPCCreate_World:TooManyOutputArguments","Too many output arguments for entry-point petscPCCreate_World.");
        /* Call the API function. */
        petscPCCreate_World_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscPCCreate:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscPCCreate.");
