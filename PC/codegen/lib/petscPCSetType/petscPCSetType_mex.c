/*
 * PC/codegen/lib/petscPCSetType/petscPCSetType_mex.c
 *
 * Auxiliary code for mexFunction of petscPCSetType
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {PetscPC, PetscPCType} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscPCSetType_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscPCSetType_initialize();

    petscPCSetType(&pc, &type, errCode, toplevel);

    petscPCSetType_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscPCSetType:TooManyOutputArguments","Too many output arguments for entry-point petscPCSetType.");
        /* Call the API function. */
        petscPCSetType_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscPCSetType:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscPCSetType.");
