/*
 * PC/codegen/lib/petscPCGetOptionsPrefix/petscPCGetOptionsPrefix_mex.c
 *
 * Auxiliary code for mexFunction of petscPCGetOptionsPrefix
 *
...
void petscPCGetOptionsPrefix_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscPCGetOptionsPrefix:WrongInputType",
            "Input argument pc has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscPCGetOptionsPrefix:InputStructWrongFields",
            "Input argument pc has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscPCGetOptionsPrefix:WrongSizeOfInputArg",
            "Argument pc must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscPCGetOptionsPrefix:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscPCGetOptionsPrefix_initialize();

    petscPCGetOptionsPrefix(&pc, &str, errCode, toplevel);

    petscPCGetOptionsPrefix_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscPCGetOptionsPrefix:TooManyOutputArguments","Too many output arguments for entry-point petscPCGetOptionsPrefix.");
        /* Call the API function. */
        petscPCGetOptionsPrefix_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscPCGetOptionsPrefix:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscPCGetOptionsPrefix.");
