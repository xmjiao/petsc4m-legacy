/*
 * PC/codegen/lib/petscPCSetFromOptions/petscPCSetFromOptions_mex.c
 *
 * Auxiliary code for mexFunction of petscPCSetFromOptions
 *
...
void petscPCSetFromOptions_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscPCSetFromOptions:WrongInputType",
            "Input argument pc has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscPCSetFromOptions:InputStructWrongFields",
            "Input argument pc has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscPCSetFromOptions:WrongSizeOfInputArg",
            "Argument pc must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscPCSetFromOptions:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscPCSetFromOptions_initialize();

    petscPCSetFromOptions(&pc, errCode, toplevel);

    petscPCSetFromOptions_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscPCSetFromOptions:TooManyOutputArguments","Too many output arguments for entry-point petscPCSetFromOptions.");
        /* Call the API function. */
        petscPCSetFromOptions_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscPCSetFromOptions:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscPCSetFromOptions.");
