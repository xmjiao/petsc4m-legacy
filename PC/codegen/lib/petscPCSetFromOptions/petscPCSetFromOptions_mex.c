...
static void __petscPCSetFromOptions_api(mxArray **plhs, const mxArray ** prhs) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscPCSetFromOptions:TooManyOutputArguments",
                "Too many output arguments for entry-point petscPCSetFromOptions.\n");
        /* Call the API function. */
        __petscPCSetFromOptions_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscPCSetFromOptions:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscPCSetFromOptions.");
