/*
 * PC/codegen/lib/petscPCFactorGetMatrix/petscPCFactorGetMatrix_mex.c
 *
 * Auxiliary code for mexFunction of petscPCFactorGetMatrix
 *
...
void petscPCFactorGetMatrix_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscPCFactorGetMatrix:WrongInputType",
            "Input argument pc has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscPCFactorGetMatrix:InputStructWrongFields",
            "Input argument pc has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscPCFactorGetMatrix:WrongSizeOfInputArg",
            "Argument pc must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscPCFactorGetMatrix:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscPCFactorGetMatrix_initialize();

    petscPCFactorGetMatrix(&pc, &mat, errCode, toplevel);

    petscPCFactorGetMatrix_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscPCFactorGetMatrix:TooManyOutputArguments","Too many output arguments for entry-point petscPCFactorGetMatrix.");
        /* Call the API function. */
        petscPCFactorGetMatrix_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscPCFactorGetMatrix:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscPCFactorGetMatrix.");
