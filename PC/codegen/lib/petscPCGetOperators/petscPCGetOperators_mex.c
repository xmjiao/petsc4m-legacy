/*
 * PC/codegen/lib/petscPCGetOperators/petscPCGetOperators_mex.c
 *
 * Auxiliary code for mexFunction of petscPCGetOperators
 *
...
static void __petscPCGetOperators_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       pc;
    M2C_OpaqueType       Amat;
    M2C_OpaqueType       Pmat;
    int32_T             *errCode;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("petscPCGetOperators:WrongInputType",
            "Input argument pc has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscPCGetOperators:WrongSizeOfInputArg",
            "Argument pc should be a scalar.");
    marshallin_const_M2C_OpaqueType(&pc, prhs[0], "pc");
    prealloc_M2C_OpaqueType(&Amat);
    prealloc_M2C_OpaqueType(&Pmat);
    ...
    petscPCGetOperators(&pc, &Amat, &Pmat, errCode, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&pc);
    plhs[0] = marshallout_M2C_OpaqueType(&Amat);
    plhs[1] = marshallout_M2C_OpaqueType(&Pmat);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 4)
            mexErrMsgIdAndTxt("petscPCGetOperators:TooManyOutputArguments",
                "Too many output arguments for entry-point petscPCGetOperators.\n");
        /* Call the API function. */
        __petscPCGetOperators_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscPCGetOperators:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscPCGetOperators.");
