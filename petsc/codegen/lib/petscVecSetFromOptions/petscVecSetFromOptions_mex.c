/*
 * petsc/codegen/lib/petscVecSetFromOptions/petscVecSetFromOptions_mex.c
 *
 * Auxiliary code for mexFunction of petscVecSetFromOptions
 *
...
void petscVecSetFromOptions_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscVecSetFromOptions:WrongInputType",
            "Input argument vec has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscVecSetFromOptions:InputStructWrongFields",
            "Input argument vec has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscVecSetFromOptions:WrongSizeOfInputArg",
            "Argument vec must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscVecSetFromOptions:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscVecSetFromOptions_initialize();

    petscVecSetFromOptions(&vec, errCode, toplevel);

    petscVecSetFromOptions_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscVecSetFromOptions:TooManyOutputArguments","Too many output arguments for entry-point petscVecSetFromOptions.");
        /* Call the API function. */
        petscVecSetFromOptions_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscVecSetFromOptions:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscVecSetFromOptions.");
