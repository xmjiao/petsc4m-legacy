/*
 * petsc/codegen/lib/petscKSPGetResidualNorm/petscKSPGetResidualNorm_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPGetResidualNorm
 *
...
void petscKSPGetResidualNorm_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscKSPGetResidualNorm:WrongInputType",
            "Input argument ksp has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscKSPGetResidualNorm:InputStructWrongFields",
            "Input argument ksp has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscKSPGetResidualNorm:WrongSizeOfInputArg",
            "Argument ksp must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscKSPGetResidualNorm:WrongInputStruct",
    ...
    *(void**)&ksp.type = mxCalloc(1, sizeof(emxArray__common));
    ...
    {mwSize l_size[] = {1, 1};
    *(void **)&rnorm = prealloc_mxArray((mxArray**)&plhs[0], mxDOUBLE_CLASS, 2, l_size); }
    {mwSize l_size[] = {1, 1};
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPGetResidualNorm_initialize();

    petscKSPGetResidualNorm(&ksp, rnorm, errCode, toplevel);

    petscKSPGetResidualNorm_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPGetResidualNorm:TooManyOutputArguments","Too many output arguments for entry-point petscKSPGetResidualNorm.");
        /* Call the API function. */
        petscKSPGetResidualNorm_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetResidualNorm:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPGetResidualNorm.");
