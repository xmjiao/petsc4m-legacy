/*
 * petsc/codegen/lib/petscKSPGetOptionsPrefix/petscKSPGetOptionsPrefix_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPGetOptionsPrefix
 *
...
void petscKSPGetOptionsPrefix_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscKSPGetOptionsPrefix:WrongInputType",
            "Input argument ksp has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscKSPGetOptionsPrefix:InputStructWrongFields",
            "Input argument ksp has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscKSPGetOptionsPrefix:WrongSizeOfInputArg",
            "Argument ksp must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscKSPGetOptionsPrefix:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPGetOptionsPrefix_initialize();

    petscKSPGetOptionsPrefix(&ksp, &str, errCode, toplevel);

    petscKSPGetOptionsPrefix_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPGetOptionsPrefix:TooManyOutputArguments","Too many output arguments for entry-point petscKSPGetOptionsPrefix.");
        /* Call the API function. */
        petscKSPGetOptionsPrefix_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetOptionsPrefix:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPGetOptionsPrefix.");
