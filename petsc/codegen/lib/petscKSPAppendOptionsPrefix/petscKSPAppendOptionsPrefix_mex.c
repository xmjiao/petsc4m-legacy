/*
 * petsc/codegen/lib/petscKSPAppendOptionsPrefix/petscKSPAppendOptionsPrefix_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPAppendOptionsPrefix
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {PetscKSP, coder.typeof(char(0), [1, inf])}  enableInline=1 withPetsc=1
 *
 */
...
void petscKSPAppendOptionsPrefix_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscKSPAppendOptionsPrefix:WrongInputType",
            "Input argument ksp has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscKSPAppendOptionsPrefix:InputStructWrongFields",
            "Input argument ksp has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscKSPAppendOptionsPrefix:WrongSizeOfInputArg",
            "Argument ksp must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscKSPAppendOptionsPrefix:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPAppendOptionsPrefix_initialize();

    petscKSPAppendOptionsPrefix(&ksp, &prefix, errCode, toplevel);

    petscKSPAppendOptionsPrefix_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscKSPAppendOptionsPrefix:TooManyOutputArguments","Too many output arguments for entry-point petscKSPAppendOptionsPrefix.");
        /* Call the API function. */
        petscKSPAppendOptionsPrefix_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPAppendOptionsPrefix:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPAppendOptionsPrefix.");
