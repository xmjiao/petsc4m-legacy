/*
 * petsc/codegen/lib/petscMatGetLocalSize/petscMatGetLocalSize_mex.c
 *
 * Auxiliary code for mexFunction of petscMatGetLocalSize
 *
...
void petscMatGetLocalSize_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscMatGetLocalSize:WrongInputType",
            "Input argument mat has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatGetLocalSize:InputStructWrongFields",
            "Input argument mat has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscMatGetLocalSize:WrongSizeOfInputArg",
            "Argument mat must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatGetLocalSize:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[3], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatGetLocalSize_initialize();

    petscMatGetLocalSize(&mat, m, n, errCode, toplevel);

    petscMatGetLocalSize_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 4)
            mexErrMsgIdAndTxt("petscMatGetLocalSize:TooManyOutputArguments","Too many output arguments for entry-point petscMatGetLocalSize.");
        /* Call the API function. */
        petscMatGetLocalSize_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatGetLocalSize:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatGetLocalSize.");
