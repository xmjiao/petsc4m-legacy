/*
 * petsc/codegen/lib/petscKSPGetInitialGuessNonzero/petscKSPGetInitialGuessNonzero_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPGetInitialGuessNonzero
 *
...
void petscKSPGetInitialGuessNonzero_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscKSPGetInitialGuessNonzero:WrongInputType",
            "Input argument ksp has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscKSPGetInitialGuessNonzero:InputStructWrongFields",
            "Input argument ksp has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscKSPGetInitialGuessNonzero:WrongSizeOfInputArg",
            "Argument ksp must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscKSPGetInitialGuessNonzero:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPGetInitialGuessNonzero_initialize();

    petscKSPGetInitialGuessNonzero(&ksp, flag, errCode, toplevel);

    petscKSPGetInitialGuessNonzero_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPGetInitialGuessNonzero:TooManyOutputArguments","Too many output arguments for entry-point petscKSPGetInitialGuessNonzero.");
        /* Call the API function. */
        petscKSPGetInitialGuessNonzero_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetInitialGuessNonzero:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPGetInitialGuessNonzero.");
