/*
 * petsc/codegen/lib/petscMatSetFromOptions/petscMatSetFromOptions_mex.c
 *
 * Auxiliary code for mexFunction of petscMatSetFromOptions
 *
...
void petscMatSetFromOptions_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscMatSetFromOptions:WrongInputType",
            "Input argument mat has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatSetFromOptions:InputStructWrongFields",
            "Input argument mat has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscMatSetFromOptions:WrongSizeOfInputArg",
            "Argument mat must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatSetFromOptions:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatSetFromOptions_initialize();

    petscMatSetFromOptions(&mat, errCode, toplevel);

    petscMatSetFromOptions_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatSetFromOptions:TooManyOutputArguments","Too many output arguments for entry-point petscMatSetFromOptions.");
        /* Call the API function. */
        petscMatSetFromOptions_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatSetFromOptions:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatSetFromOptions.");
