/*
 * petsc/codegen/lib/petscVecAssemblyBegin/petscVecAssemblyBegin_mex.c
 *
 * Auxiliary code for mexFunction of petscVecAssemblyBegin
 *
...
void petscVecAssemblyBegin_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscVecAssemblyBegin:WrongInputType",
            "Input argument vec has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscVecAssemblyBegin:InputStructWrongFields",
            "Input argument vec has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscVecAssemblyBegin:WrongSizeOfInputArg",
            "Argument vec must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscVecAssemblyBegin:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscVecAssemblyBegin_initialize();

    petscVecAssemblyBegin(&vec, errCode, toplevel);

    petscVecAssemblyBegin_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscVecAssemblyBegin:TooManyOutputArguments","Too many output arguments for entry-point petscVecAssemblyBegin.");
        /* Call the API function. */
        petscVecAssemblyBegin_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscVecAssemblyBegin:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscVecAssemblyBegin.");
