/*
 * petsc/codegen/lib/petscKSPGetConvergedReason/petscKSPGetConvergedReason_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPGetConvergedReason
 *
...
void petscKSPGetConvergedReason_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscKSPGetConvergedReason:WrongInputType",
            "Input argument ksp has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscKSPGetConvergedReason:InputStructWrongFields",
            "Input argument ksp has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscKSPGetConvergedReason:WrongSizeOfInputArg",
            "Argument ksp must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscKSPGetConvergedReason:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPGetConvergedReason_initialize();

    petscKSPGetConvergedReason(&ksp, flag, errCode, toplevel);

    petscKSPGetConvergedReason_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPGetConvergedReason:TooManyOutputArguments","Too many output arguments for entry-point petscKSPGetConvergedReason.");
        /* Call the API function. */
        petscKSPGetConvergedReason_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetConvergedReason:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPGetConvergedReason.");
