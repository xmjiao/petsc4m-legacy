...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscVecSet:TooManyOutputArguments",
                "Too many output arguments for entry-point petscVecSet.\n");
        /* Call the API function. */
        __petscVecSet_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscVecSet:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscVecSet.");
