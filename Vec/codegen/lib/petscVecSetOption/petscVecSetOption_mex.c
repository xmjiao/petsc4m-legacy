/*
 * Vec/codegen/lib/petscVecSetOption/petscVecSetOption_mex.c
 *
 * Auxiliary code for mexFunction of petscVecSetOption
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {PetscVec, int32(0), int32(0)} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscVecSetOption_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscVecSetOption:WrongInputType",
            "Input argument vec has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscVecSetOption:InputStructWrongFields",
            "Input argument vec has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscVecSetOption:WrongSizeOfInputArg",
            "Argument vec must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscVecSetOption:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscVecSetOption_initialize();

    petscVecSetOption(&vec, op, flg, errCode, toplevel);

    petscVecSetOption_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscVecSetOption:TooManyOutputArguments","Too many output arguments for entry-point petscVecSetOption.");
        /* Call the API function. */
        petscVecSetOption_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscVecSetOption:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscVecSetOption.");
