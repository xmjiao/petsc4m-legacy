...
void petscVecAYPX_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscVecAYPX_initialize();

    petscVecAYPX(&y, a, &x, errCode, toplevel);

    petscVecAYPX_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscVecAYPX:TooManyOutputArguments","Too many output arguments for entry-point petscVecAYPX.");
        /* Call the API function. */
        petscVecAYPX_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscVecAYPX:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscVecAYPX.");
