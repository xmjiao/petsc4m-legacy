/*
 * Vec/codegen/lib/petscVecShift/petscVecShift_mex.c
 *
 * Auxiliary code for mexFunction of petscVecShift
 *
...
static void __petscVecShift_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       v;
    real64_T             a;
    int32_T             *errCode;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("petscVecShift:WrongInputType",
            "Input argument v has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscVecShift:WrongSizeOfInputArg",
    ...
    petscVecShift(&v, a, errCode, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscVecShift:TooManyOutputArguments",
                "Too many output arguments for entry-point petscVecShift.\n");
        /* Call the API function. */
        __petscVecShift_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscVecShift:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscVecShift.");
