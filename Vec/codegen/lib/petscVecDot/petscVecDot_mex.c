...
void petscVecDot_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscVecDot_initialize();

    petscVecDot(&x, &y, val, errCode, toplevel);

    petscVecDot_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscVecDot:TooManyOutputArguments","Too many output arguments for entry-point petscVecDot.");
        /* Call the API function. */
        petscVecDot_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscVecDot:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscVecDot.");
