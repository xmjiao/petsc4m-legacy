...
static void __petscVecAssemblyEnd_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            vec;
    int32_T             *errCode;
    boolean_T           *toplevel;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscVecAssemblyEnd:TooManyOutputArguments",
                "Too many output arguments for entry-point petscVecAssemblyEnd.\n");
        /* Call the API function. */
        __petscVecAssemblyEnd_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscVecAssemblyEnd:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscVecAssemblyEnd.");
