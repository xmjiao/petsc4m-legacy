...
static void __petscVecCopy_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            x;
    struct0_T            y;
    int32_T             *errCode;
    boolean_T           *toplevel;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscVecCopy:TooManyOutputArguments",
                "Too many output arguments for entry-point petscVecCopy.\n");
        /* Call the API function. */
        __petscVecCopy_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscVecCopy:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscVecCopy.");
