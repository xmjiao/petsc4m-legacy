...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 6)
            mexErrMsgIdAndTxt("petscKSPGetTolerances:TooManyOutputArguments",
                "Too many output arguments for entry-point petscKSPGetTolerances.\n");
        /* Call the API function. */
        __petscKSPGetTolerances_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetTolerances:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscKSPGetTolerances.");
