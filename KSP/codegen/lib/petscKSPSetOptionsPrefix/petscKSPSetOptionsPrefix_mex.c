...
static void __petscKSPSetOptionsPrefix_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            ksp;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscKSPSetOptionsPrefix:TooManyOutputArguments",
                "Too many output arguments for entry-point petscKSPSetOptionsPrefix.\n");
        /* Call the API function. */
        __petscKSPSetOptionsPrefix_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPSetOptionsPrefix:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscKSPSetOptionsPrefix.");
