/*
 * KSP/codegen/lib/petscKSPCreate/petscKSPCreate_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPCreate
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {MPI_Comm} petscKSPCreate_World -args {} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscKSPCreate_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPCreate_initialize();

    petscKSPCreate(&comm, &ksp, errCode, toplevel);

    petscKSPCreate_terminate();
...
void petscKSPCreate_World_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPCreate_World(&ksp, errCode, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPCreate:TooManyOutputArguments","Too many output arguments for entry-point petscKSPCreate.");
        /* Call the API function. */
        petscKSPCreate_api(prhs, (const mxArray**)outputs);
    }
    else if (nrhs == 0) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPCreate_World:TooManyOutputArguments","Too many output arguments for entry-point petscKSPCreate_World.");
        /* Call the API function. */
        petscKSPCreate_World_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPCreate:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPCreate.");
