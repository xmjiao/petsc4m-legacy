...
void petscKSPSetTolerances(const struct0_T *ksp, double rtol, double arg2,
  double dtol, int maxits, int *errCode, boolean_T *toplevel)
{
  ...
  *errCode = KSPSetTolerances(t_ksp, rtol, arg2, dtol, maxits);
  *toplevel = true;
...
void petscKSPSetTolerances_3args(const struct0_T *ksp, double rtols, int maxits,
  int *errCode, boolean_T *toplevel)
{
  ...
  abstol = (PETSC_DEFAULT);
  dtol = (PETSC_DEFAULT);
  *errCode = KSPSetTolerances(t_ksp, rtols, abstol, dtol, maxits);
  emxFree_uint8_T(&data);
  if (*errCode != 0) {
    b_m2c_error(*errCode);
  }
