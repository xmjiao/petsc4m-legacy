/*
 * KSP/codegen/lib/petscKSPGetResidualHistory/petscKSPGetResidualHistory_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPGetResidualHistory
 *
...
void petscKSPGetResidualHistory_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscKSPGetResidualHistory:WrongInputType",
            "Input argument ksp has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscKSPGetResidualHistory:InputStructWrongFields",
            "Input argument ksp has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscKSPGetResidualHistory:WrongSizeOfInputArg",
            "Argument ksp must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscKSPGetResidualHistory:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPGetResidualHistory_initialize();

    petscKSPGetResidualHistory(&ksp, &reshis, errCode, toplevel);

    petscKSPGetResidualHistory_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPGetResidualHistory:TooManyOutputArguments","Too many output arguments for entry-point petscKSPGetResidualHistory.");
        /* Call the API function. */
        petscKSPGetResidualHistory_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetResidualHistory:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPGetResidualHistory.");
