...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPGetResidualHistory:TooManyOutputArguments",
                "Too many output arguments for entry-point petscKSPGetResidualHistory.\n");
        /* Call the API function. */
        __petscKSPGetResidualHistory_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetResidualHistory:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscKSPGetResidualHistory.");
