...
static void b_m2c_error(int varargin_3)
{
  ...
  fmt = "KSPSetNormType returned error code %d\n";
...
void petscKSPSetNormType(const M2C_OpaqueType *ksp, int normtype, int *errCode,
                         boolean_T *toplevel)
{
  ...
  *errCode = KSPSetNormType(c_ksp, normtype);
  *toplevel = true;
  if (*errCode != 0) {
    b_m2c_error(*errCode);
  }
...
void petscKSPSetNormType_initialize(void)
{
}

void petscKSPSetNormType_terminate(void)
{
}
