...
void petscKSPSetNormType_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscKSPSetNormType:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscKSPSetNormType:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPSetNormType_initialize();

    petscKSPSetNormType(&ksp, normtype, errCode, toplevel);

    petscKSPSetNormType_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscKSPSetNormType:TooManyOutputArguments","Too many output arguments for entry-point petscKSPSetNormType.");
        /* Call the API function. */
        petscKSPSetNormType_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPSetNormType:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPSetNormType.");
