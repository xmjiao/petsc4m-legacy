...
static void __petscKSPSolve_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            ksp;
    struct0_T            b;
    struct0_T            x;
    int32_T             *errCode;
...
static void __petscKSPSolve_2args_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            ksp;
    struct0_T            b;
    int32_T             *errCode;
    boolean_T           *toplevel;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscKSPSolve:TooManyOutputArguments",
                "Too many output arguments for entry-point petscKSPSolve.\n");
        /* Call the API function. */
        __petscKSPSolve_api(outputs, prhs);
    }
    else if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscKSPSolve_2args:TooManyOutputArguments",
                "Too many output arguments for entry-point petscKSPSolve_2args.\n");
        /* Call the API function. */
        __petscKSPSolve_2args_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPSolve:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscKSPSolve.");
