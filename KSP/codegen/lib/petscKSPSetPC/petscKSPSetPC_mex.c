...
void petscKSPSetPC_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPSetPC_initialize();

    petscKSPSetPC(&ksp, &pc, errCode, toplevel);

    petscKSPSetPC_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscKSPSetPC:TooManyOutputArguments","Too many output arguments for entry-point petscKSPSetPC.");
        /* Call the API function. */
        petscKSPSetPC_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPSetPC:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPSetPC.");
