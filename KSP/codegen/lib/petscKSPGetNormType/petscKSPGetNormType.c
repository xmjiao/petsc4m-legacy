...
static void b_m2c_error(int varargin_3)
{
  ...
  fmt = "KSPGetNormType returned error code %d\n";
...
void petscKSPGetNormType(const M2C_OpaqueType *ksp, int *normtype, int *errCode,
                         boolean_T *toplevel)
{
  ...
  *errCode = KSPGetNormType(c_ksp, normtype);
  *toplevel = true;
  if (*errCode != 0) {
    b_m2c_error(*errCode);
  }
...
void petscKSPGetNormType_initialize(void)
{
}

void petscKSPGetNormType_terminate(void)
{
}
