/*
 * KSP/codegen/lib/petscKSPGetNormType/petscKSPGetNormType_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPGetNormType
 *
...
void petscKSPGetNormType_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscKSPGetNormType:WrongInputType",
            "Input argument ksp has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscKSPGetNormType:InputStructWrongFields",
            "Input argument ksp has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscKSPGetNormType:WrongSizeOfInputArg",
            "Argument ksp must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscKSPGetNormType:WrongInputStruct",
    ...
    *(void**)&ksp.type = mxCalloc(1, sizeof(emxArray__common));
    ...
    {mwSize l_size[] = {1, 1};
    *(void **)&normtype = prealloc_mxArray((mxArray**)&plhs[0], mxINT32_CLASS, 2, l_size); }
    {mwSize l_size[] = {1, 1};
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPGetNormType_initialize();

    petscKSPGetNormType(&ksp, normtype, errCode, toplevel);

    petscKSPGetNormType_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPGetNormType:TooManyOutputArguments","Too many output arguments for entry-point petscKSPGetNormType.");
        /* Call the API function. */
        petscKSPGetNormType_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetNormType:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPGetNormType.");
