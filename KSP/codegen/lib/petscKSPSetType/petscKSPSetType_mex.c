/*
 * KSP/codegen/lib/petscKSPSetType/petscKSPSetType_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPSetType
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {PetscKSP, PetscKSPType} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscKSPSetType_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPSetType_initialize();

    petscKSPSetType(&ksp, &type, errCode, toplevel);

    petscKSPSetType_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscKSPSetType:TooManyOutputArguments","Too many output arguments for entry-point petscKSPSetType.");
        /* Call the API function. */
        petscKSPSetType_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPSetType:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPSetType.");
