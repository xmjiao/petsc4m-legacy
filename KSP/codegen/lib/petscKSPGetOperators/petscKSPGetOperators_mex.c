/*
 * KSP/codegen/lib/petscKSPGetOperators/petscKSPGetOperators_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPGetOperators
 *
 * C source code generated by m2c.
 * %#m2c options:ebedc602849019eeeb1acf7fa486f6be
 *
 */
...
static void __petscKSPGetOperators_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       pc;
    struct0_T            Amat;
    struct0_T            Pmat;
    int32_T             *errCode;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("petscKSPGetOperators:WrongInputType",
            "Input argument pc has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscKSPGetOperators:WrongSizeOfInputArg",
            "Argument pc should be a scalar.");
    marshallin_const_M2C_OpaqueType(&pc, prhs[0], "pc");
    prealloc_struct0_T(&Amat);
    prealloc_struct0_T(&Pmat);
    ...
    petscKSPGetOperators(&pc, &Amat, &Pmat, errCode, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&pc);
    plhs[0] = marshallout_struct0_T(&Amat);
    plhs[1] = marshallout_struct0_T(&Pmat);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 4)
            mexErrMsgIdAndTxt("petscKSPGetOperators:TooManyOutputArguments",
                "Too many output arguments for entry-point petscKSPGetOperators.\n");
        /* Call the API function. */
        __petscKSPGetOperators_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetOperators:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscKSPGetOperators.");
