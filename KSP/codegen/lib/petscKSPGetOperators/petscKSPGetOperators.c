#include "petscKSPGetOperators.h"
...
static void b_m2c_error(int varargin_3)
{
  ...
  fmt = "KSPGetOperators returned error code %d\n";
...
void petscKSPGetOperators(const M2C_OpaqueType *pc, M2C_OpaqueType *Amat,
                          ...
                          boolean_T *toplevel)
{
  ...
  ksp = *(KSP *)(&pc->data->data[0]);
  *errCode = KSPGetOperators(ksp, &arg, &b_arg);
  sizepe = sizeof(Mat);
  i = Amat->data->size[0];
...
void petscKSPGetOperators_initialize(void)
{
}

void petscKSPGetOperators_terminate(void)
{
}
