/*
 * KSP/codegen/lib/petscKSPSetPCSide/petscKSPSetPCSide_mex.c
 *
 * Auxiliary code for mexFunction of petscKSPSetPCSide
 *
...
void petscKSPSetPCSide_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscKSPSetPCSide:WrongInputType",
            "Input argument ksp has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscKSPSetPCSide:InputStructWrongFields",
            "Input argument ksp has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscKSPSetPCSide:WrongSizeOfInputArg",
            "Argument ksp must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscKSPSetPCSide:WrongInputStruct",
    ...
    *(void**)&ksp.type = mxCalloc(1, sizeof(emxArray__common));
    ...
    if (mxGetNumberOfElements(prhs[1]) && mxGetClassID(prhs[1]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("petscKSPSetPCSide:WrongInputType",
            "Input argument side has incorrect data type. int32 is expected.");
    if (mxGetNumberOfElements(prhs[1]) != 1)
        mexErrMsgIdAndTxt("petscKSPSetPCSide:WrongSizeOfInputArg",
            "Argument side should be a scalar.");
    side = *(int32_T*)mxGetData(prhs[1]);
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscKSPSetPCSide_initialize();

    petscKSPSetPCSide(&ksp, side, errCode, toplevel);

    petscKSPSetPCSide_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscKSPSetPCSide:TooManyOutputArguments","Too many output arguments for entry-point petscKSPSetPCSide.");
        /* Call the API function. */
        petscKSPSetPCSide_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPSetPCSide:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscKSPSetPCSide.");
