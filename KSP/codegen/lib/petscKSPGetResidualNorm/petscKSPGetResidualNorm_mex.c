...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscKSPGetResidualNorm:TooManyOutputArguments",
                "Too many output arguments for entry-point petscKSPGetResidualNorm.\n");
        /* Call the API function. */
        __petscKSPGetResidualNorm_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscKSPGetResidualNorm:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscKSPGetResidualNorm.");
