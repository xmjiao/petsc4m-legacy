  ...
  emxArray_char_T *pctype, const emxArray_char_T *pcopt, KSP *ksp, double *time)
{
  ...
  *ksp = t_ksp;
        ...
        if (p) {
          ...
          if (p) {
            ...
            if (pcopt->data[pcopt->size[1] - 1] != '\x00') {
              i3 = pcopt0->size[0] * pcopt0->size[1];
              pcopt0->size[0] = 1;
              pcopt0->size[1] = pcopt->size[1] + 1;
              emxEnsureCapacity((emxArray__common *)pcopt0, i3, (int)sizeof(char));
              k = pcopt->size[1];
              for (i3 = 0; i3 < k; i3++) {
                pcopt0->data[pcopt0->size[0] * i3] = pcopt->data[pcopt->size[0] *
                  i3];
              }
  ...
  if ((!(ksptype->size[1] == 0)) && ((unsigned char)ksptype->data[ksptype->size
       [1] - 1] != 0)) {
    i3 = ksptype0->size[0] * ksptype0->size[1];
    ksptype0->size[0] = 1;
    ksptype0->size[1] = ksptype->size[1] + 1;
    emxEnsureCapacity((emxArray__common *)ksptype0, i3, (int)sizeof(char));
    k = ksptype->size[1];
    for (i3 = 0; i3 < k; i3++) {
      ksptype0->data[ksptype0->size[0] * i3] = ksptype->data[ksptype->size[0] *
        i3];
    }
    ...
    for (i3 = 0; i3 < k; i3++) {
      ksptype0->data[i3] = ksptype->data[i3];
    }
  }
  ...
  *iter, emxArray_real_T *reshis, double times[2])
{
  ...
  b_petscKSPDriver(ksp, bVec, xVec, rtol, maxiter, x0Vec, &b_flag, &b_relres,
  ...
  *reshis, double times[2])
{
  ...
  MPI_Barrier(t_comm);
  secs = MPI_Wtime();
  petscKSPDriver(t_ksp, bVec, t_vec_out, NULL, &b_flag, &b_relres, &b_iter,
                 reshis, &time_solve);
  KSPDestroy(&t_ksp);
  t_mat = AMat;
  MatDestroy(&t_mat);
  ...
  *iter, emxArray_real_T *reshis, double times[2])
{
  ...
  petscKSPDriver(ksp, bVec, t_vec_out, NULL, &b_flag, &b_relres, &b_iter, reshis,
                 &time_solve);
  t_ksp = ksp;
  KSPDestroy(&t_ksp);
  t_mat = AMat;
  ...
  *flag, double *relres, int *iter, emxArray_real_T *reshis, double times[2])
{
  ...
  VecDuplicate(bVec, &t_vec_out);
  petscKSPSetup(AMat, solver, &ksp, &time_setup);
  b_petscKSPDriver(ksp, bVec, t_vec_out, rtol, maxiter, NULL, &b_flag, &b_relres,
                   &b_iter, reshis, &time_solve);
  t_ksp = ksp;
  KSPDestroy(&t_ksp);
  t_mat = AMat;
  ...
  emxArray_real_T *reshis, double times[2])
{
  ...
  MPI_Barrier(t_comm);
  secs = MPI_Wtime();
  b_petscKSPDriver(t_ksp, bVec, t_vec_out, rtol, maxiter, NULL, &b_flag,
                   &b_relres, &b_iter, reshis, &time_solve);
  KSPDestroy(&t_ksp);
  t_mat = AMat;
  MatDestroy(&t_mat);
  t_vec = bVec;
  ...
  *relres, int *iter, emxArray_real_T *reshis, double times[2])
{
  ...
  petscSolveHdls(AMat, bVec, t_vec_out, solver, rtol, maxiter, pctype, solpack,
                 NULL, &b_flag, &b_relres, &b_iter, reshis, times);
  t_mat = AMat;
  MatDestroy(&t_mat);
  t_vec = bVec;
  VecDestroy(&t_vec);
