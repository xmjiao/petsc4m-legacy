/*
 * Mat/codegen/lib/petscMatMultHermitianTranspose/petscMatMultHermitianTranspose_mex.c
 *
 * Auxiliary code for mexFunction of petscMatMultHermitianTranspose
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {PetscMat, PetscVec, PetscVec} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscMatMultHermitianTranspose_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscMatMultHermitianTranspose:WrongInputType",
            "Input argument A has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatMultHermitianTranspose:InputStructWrongFields",
            "Input argument A has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscMatMultHermitianTranspose:WrongSizeOfInputArg",
            "Argument A must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatMultHermitianTranspose:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatMultHermitianTranspose_initialize();

    petscMatMultHermitianTranspose(&A, &x, &y, errCode, toplevel);

    petscMatMultHermitianTranspose_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatMultHermitianTranspose:TooManyOutputArguments","Too many output arguments for entry-point petscMatMultHermitianTranspose.");
        /* Call the API function. */
        petscMatMultHermitianTranspose_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatMultHermitianTranspose:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatMultHermitianTranspose.");
