...
void petscMatCreate_World(struct0_T *mat, int *errCode, boolean_T *topleve)
{
  ...
  int i;
  emxInit_uint8_T(&data0, 1);
  b_errCode = MatCreate(PETSC_COMM_WORLD, &t_mat);
  sizepe = sizeof(Mat);
  i2 = data0->size[0];
  data0->size[0] = sizepe;
  ...
  i2 = mat->data->size[0];
  mat->data->size[0] = data0->size[0];
  emxEnsureCapacity((emxArray__common *)mat->data, i2, (int)sizeof(unsigned char));
  loop_ub = data0->size[0];
  for (i2 = 0; i2 < loop_ub; i2++) {
    mat->data->data[i2] = data0->data[i2];
  }

  emxFree_uint8_T(&data0);
  i2 = mat->type->size[0] * mat->type->size[1];
  mat->type->size[0] = 1;
  mat->type->size[1] = 3;
  emxEnsureCapacity((emxArray__common *)mat->type, i2, (int)sizeof(char));
  for (i2 = 0; i2 < 3; i2++) {
    mat->type->data[i2] = t0_type[i2];
  }
