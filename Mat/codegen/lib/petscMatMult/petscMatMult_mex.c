...
static void __petscMatMult_api(mxArray **plhs, const mxArray ** prhs) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatMult:TooManyOutputArguments",
                "Too many output arguments for entry-point petscMatMult.\n");
        /* Call the API function. */
        __petscMatMult_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscMatMult:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscMatMult.");
