...
void petscMatMult(const struct0_T *A, const struct0_T *x, const struct0_T *y,
                  int *errCode, boolean_T *toplevel)
{
  ...
  if (!p) {
    ...
    for (k = 0; k < loop_ub; k++) {
      b_A->data[b_A->size[0] * k] = x->type->data[x->type->size[0] * k];
    }

    b_A->data[b_A->size[0] * x->type->size[1]] = '\x00';
    b_m2c_error(b_A);
  }
  ...
  if (!p) {
    ...
    for (k = 0; k < loop_ub; k++) {
      b_A->data[b_A->size[0] * k] = y->type->data[y->type->size[0] * k];
    }

    b_A->data[b_A->size[0] * y->type->size[1]] = '\x00';
    b_m2c_error(b_A);
  }
