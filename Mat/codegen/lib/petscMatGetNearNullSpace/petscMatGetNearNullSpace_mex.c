/*
 * Mat/codegen/lib/petscMatGetNearNullSpace/petscMatGetNearNullSpace_mex.c
 *
 * Auxiliary code for mexFunction of petscMatGetNearNullSpace
 *
...
void petscMatGetNearNullSpace_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscMatGetNearNullSpace:WrongInputType",
            "Input argument mat has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatGetNearNullSpace:InputStructWrongFields",
            "Input argument mat has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscMatGetNearNullSpace:WrongSizeOfInputArg",
            "Argument mat must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatGetNearNullSpace:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatGetNearNullSpace_initialize();

    petscMatGetNearNullSpace(&mat, &nullsp, errCode, toplevel);

    petscMatGetNearNullSpace_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscMatGetNearNullSpace:TooManyOutputArguments","Too many output arguments for entry-point petscMatGetNearNullSpace.");
        /* Call the API function. */
        petscMatGetNearNullSpace_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatGetNearNullSpace:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatGetNearNullSpace.");
