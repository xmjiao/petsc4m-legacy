...
void petscMatMultAdd_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatMultAdd:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatMultAdd:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatMultAdd_initialize();

    petscMatMultAdd(&A, &v1, &v2, &v3, errCode, toplevel);

    petscMatMultAdd_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 4) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatMultAdd:TooManyOutputArguments","Too many output arguments for entry-point petscMatMultAdd.");
        /* Call the API function. */
        petscMatMultAdd_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatMultAdd:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatMultAdd.");
