 ...
 * %#m2c options: codegenArgs=-args {PetscMat, 0, PetscMat, int32(0)} petscMatAYPX_3args -args {PetscMat, 0, PetscMat} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscMatAYPX_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatAYPX:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatAYPX:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatAYPX_initialize();

    petscMatAYPX(&Y, a, &X, mstr, errCode, toplevel);

    petscMatAYPX_terminate();
...
void petscMatAYPX_3args_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscMatAYPX_3args:WrongInputType",
            "Input argument Y has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatAYPX_3args:InputStructWrongFields",
            "Input argument Y has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
    ...
    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatAYPX_3args:WrongInputStruct",
            "Input argument Y does not have the field data.");
    if (mxGetNumberOfElements(_sub_mx1) && mxGetClassID(_sub_mx1) != mxUINT8_CLASS)
        mexErrMsgIdAndTxt("petscMatAYPX_3args:WrongInputType",
            "Input argument Y.data has incorrect data type. uint8 is expected.");
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 4) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatAYPX:TooManyOutputArguments","Too many output arguments for entry-point petscMatAYPX.");
        /* Call the API function. */
        petscMatAYPX_api(prhs, (const mxArray**)outputs);
    }
    else if (nrhs == 3) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatAYPX_3args:TooManyOutputArguments","Too many output arguments for entry-point petscMatAYPX_3args.");
        /* Call the API function. */
        petscMatAYPX_3args_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatAYPX:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatAYPX.");
