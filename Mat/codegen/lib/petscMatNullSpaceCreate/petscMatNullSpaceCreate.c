  ...
  struct0_T *vecs, struct0_T *nullsp, int *errCode, boolean_T *toplevel)
{
  ...
  i0 = data0->size[0];
  data0->size[0] = vecs->data->size[0];
  emxEnsureCapacity((emxArray__common *)data0, i0, (int)sizeof(unsigned char));
  k = vecs->data->size[0];
  for (i0 = 0; i0 < k; i0++) {
    data0->data[i0] = vecs->data->data[i0];
  }

  t_vecs = *(Vec**)(&data0->data[0]);
  *errCode = MatNullSpaceCreate(t_comm, has_cnst, n, t_vecs, &t_nullsp);
  *toplevel = true;
  ...
  struct0_T *nullsp, int *errCode, boolean_T *toplevel)
{
  ...
  int sizepe;
  char t0_type[12];
  static const char cv3[12] = { 'M', 'a', 't', 'N', 'u', 'l', 'l', 'S', 'p', 'a',
    'c', 'e' };
  ...
  if (!p) {
    ...
    b_comm->size[0] = 1;
    b_comm->size[1] = comm->type->size[1] + 1;
    emxEnsureCapacity((emxArray__common *)b_comm, i2, (int)sizeof(char));
    k = comm->type->size[1];
    for (i2 = 0; i2 < k; i2++) {
  ...
  k = comm->data->size[0];
  for (i2 = 0; i2 < k; i2++) {
    data0->data[i2] = comm->data->data[i2];
  }
  ...
  i2 = nullsp->data->size[0];
  nullsp->data->size[0] = data0->size[0];
  emxEnsureCapacity((emxArray__common *)nullsp->data, i2, (int)sizeof(unsigned
    char));
  k = data0->size[0];
  for (i2 = 0; i2 < k; i2++) {
    nullsp->data->data[i2] = data0->data[i2];
  }

  emxFree_uint8_T(&data0);
  i2 = nullsp->type->size[0] * nullsp->type->size[1];
  nullsp->type->size[0] = 1;
  nullsp->type->size[1] = 12;
  emxEnsureCapacity((emxArray__common *)nullsp->type, i2, (int)sizeof(char));
  for (i2 = 0; i2 < 12; i2++) {
    nullsp->type->data[i2] = t0_type[i2];
  }
