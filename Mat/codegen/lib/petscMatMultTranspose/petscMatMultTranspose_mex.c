...
void petscMatMultTranspose_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatMultTranspose:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatMultTranspose:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatMultTranspose_initialize();

    petscMatMultTranspose(&A, &x, &y, errCode, toplevel);

    petscMatMultTranspose_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatMultTranspose:TooManyOutputArguments","Too many output arguments for entry-point petscMatMultTranspose.");
        /* Call the API function. */
        petscMatMultTranspose_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatMultTranspose:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatMultTranspose.");
