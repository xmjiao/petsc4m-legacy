...
void petscMatNorm_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatNorm:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatNorm:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatNorm_initialize();

    petscMatNorm(&A, type, nrm, errCode, toplevel);

    petscMatNorm_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscMatNorm:TooManyOutputArguments","Too many output arguments for entry-point petscMatNorm.");
        /* Call the API function. */
        petscMatNorm_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatNorm:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatNorm.");
