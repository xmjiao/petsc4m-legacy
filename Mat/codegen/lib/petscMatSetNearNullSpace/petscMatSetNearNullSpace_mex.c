/*
 * Mat/codegen/lib/petscMatSetNearNullSpace/petscMatSetNearNullSpace_mex.c
 *
 * Auxiliary code for mexFunction of petscMatSetNearNullSpace
 *
...
void petscMatSetNearNullSpace_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscMatSetNearNullSpace:WrongInputType",
            "Input argument mat has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatSetNearNullSpace:InputStructWrongFields",
            "Input argument mat has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscMatSetNearNullSpace:WrongSizeOfInputArg",
            "Argument mat must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatSetNearNullSpace:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatSetNearNullSpace_initialize();

    petscMatSetNearNullSpace(&mat, &nullSp, errCode, toplevel);

    petscMatSetNearNullSpace_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatSetNearNullSpace:TooManyOutputArguments","Too many output arguments for entry-point petscMatSetNearNullSpace.");
        /* Call the API function. */
        petscMatSetNearNullSpace_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatSetNearNullSpace:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatSetNearNullSpace.");
