...
static void __petscMatAssemblyEnd_api(mxArray **plhs, const mxArray ** prhs) {
...
static void __petscMatAssemblyEnd_Final_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    /* Invoke the target function */
    petscMatAssemblyEnd_Final(&mat, errCode, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatAssemblyEnd:TooManyOutputArguments",
                "Too many output arguments for entry-point petscMatAssemblyEnd.\n");
        /* Call the API function. */
        __petscMatAssemblyEnd_api(outputs, prhs);
    }
    else if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatAssemblyEnd_Final:TooManyOutputArguments",
                "Too many output arguments for entry-point petscMatAssemblyEnd_Final.\n");
        /* Call the API function. */
        __petscMatAssemblyEnd_Final_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscMatAssemblyEnd:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscMatAssemblyEnd.");
