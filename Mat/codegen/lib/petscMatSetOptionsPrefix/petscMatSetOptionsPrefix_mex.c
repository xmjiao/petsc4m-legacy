/*
 * Mat/codegen/lib/petscMatSetOptionsPrefix/petscMatSetOptionsPrefix_mex.c
 *
 * Auxiliary code for mexFunction of petscMatSetOptionsPrefix
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {PetscMat, coder.typeof(char(0), [1, inf])} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscMatSetOptionsPrefix_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscMatSetOptionsPrefix:WrongInputType",
            "Input argument mat has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatSetOptionsPrefix:InputStructWrongFields",
            "Input argument mat has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscMatSetOptionsPrefix:WrongSizeOfInputArg",
            "Argument mat must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatSetOptionsPrefix:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatSetOptionsPrefix_initialize();

    petscMatSetOptionsPrefix(&mat, &prefix, errCode, toplevel);

    petscMatSetOptionsPrefix_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatSetOptionsPrefix:TooManyOutputArguments","Too many output arguments for entry-point petscMatSetOptionsPrefix.");
        /* Call the API function. */
        petscMatSetOptionsPrefix_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatSetOptionsPrefix:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatSetOptionsPrefix.");
