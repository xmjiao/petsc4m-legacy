...
void petscMatSeqAIJSetPreallocationCSR(const M2C_OpaqueType *mat,
...
void petscMatSeqAIJSetPreallocationCSR_Indexonly(const M2C_OpaqueType *mat,
                                                 const emxArray_int32_T *ix,
                                                 const emxArray_int32_T *jx,
                                                 int *errCode,
                                                 boolean_T *toplevel)
{
  ...
  t_mat = *(Mat *)(&mat->data->data[0]);
  *errCode =
      MatSeqAIJSetPreallocationCSR(t_mat, &ix->data[0], &jx->data[0], NULL);
  if (*errCode != 0) {
    b_m2c_error(*errCode);
  }
  *toplevel = true;
