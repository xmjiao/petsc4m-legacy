 ...
 * %#m2c options: codegenArgs=-args {PetscMat,int32(0)} petscMatAssemblyBegin_Final -args {PetscMat} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscMatAssemblyBegin_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatAssemblyBegin:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatAssemblyBegin:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatAssemblyBegin_initialize();

    petscMatAssemblyBegin(&mat, type, errCode, toplevel);

    petscMatAssemblyBegin_terminate();
...
void petscMatAssemblyBegin_Final_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("petscMatAssemblyBegin_Final:WrongInputType",
            "Input argument mat has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("petscMatAssemblyBegin_Final:InputStructWrongFields",
            "Input argument mat has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
    ...
    _sub_mx1 = mxGetField(prhs[0], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("petscMatAssemblyBegin_Final:WrongInputStruct",
            "Input argument mat does not have the field data.");
    if (mxGetNumberOfElements(_sub_mx1) && mxGetClassID(_sub_mx1) != mxUINT8_CLASS)
        mexErrMsgIdAndTxt("petscMatAssemblyBegin_Final:WrongInputType",
            "Input argument mat.data has incorrect data type. uint8 is expected.");
    ...
    *(void**)&mat.type = mxCalloc(1, sizeof(emxArray__common));
    ...
    errCode = petscMatAssemblyBegin_Final(&mat);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatAssemblyBegin:TooManyOutputArguments","Too many output arguments for entry-point petscMatAssemblyBegin.");
        /* Call the API function. */
        petscMatAssemblyBegin_api(prhs, (const mxArray**)outputs);
    }
    else if (nrhs == 1) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("petscMatAssemblyBegin_Final:TooManyOutputArguments","Too many output arguments for entry-point petscMatAssemblyBegin_Final.");
        /* Call the API function. */
        petscMatAssemblyBegin_Final_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatAssemblyBegin:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatAssemblyBegin.");
