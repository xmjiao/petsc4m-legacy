/*
 * Mat/codegen/lib/petscMatScale/petscMatScale_mex.c
 *
 * Auxiliary code for mexFunction of petscMatScale
 *
...
static void __petscMatScale_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       mat;
    real32_T             a;
    int32_T             *errCode;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("petscMatScale:WrongInputType",
            "Input argument mat has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("petscMatScale:WrongSizeOfInputArg",
            "Argument mat should be a scalar.");
    marshallin_M2C_OpaqueType(&mat, prhs[0], "mat");
    ...
    petscMatScale(&mat, a, errCode, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("petscMatScale:TooManyOutputArguments",
                "Too many output arguments for entry-point petscMatScale.\n");
        /* Call the API function. */
        __petscMatScale_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("petscMatScale:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point petscMatScale.");
