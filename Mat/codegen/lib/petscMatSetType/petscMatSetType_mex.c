/*
 * Mat/codegen/lib/petscMatSetType/petscMatSetType_mex.c
 *
 * Auxiliary code for mexFunction of petscMatSetType
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {PetscMat, PetscMatType} enableInline=1 presVars=None dynMem=Threshold
 *
 */
...
void petscMatSetType_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    petscMatSetType_initialize();

    petscMatSetType(&mat, &type, errCode, toplevel);

    petscMatSetType_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("petscMatSetType:TooManyOutputArguments","Too many output arguments for entry-point petscMatSetType.");
        /* Call the API function. */
        petscMatSetType_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("petscMatSetType:WrongNumberOfInputs","Incorrect number of input variables for entry-point petscMatSetType.");
