...
static void b_m2c_error(int varargin_3)
{
  ...
  fmt = "MatSetType returned error code %d\n";
...
void petscMatSetType(const M2C_OpaqueType *mat, const emxArray_char_T *type,
                     int *errCode, boolean_T *toplevel)
{
  ...
  *errCode = MatSetType(t_mat, &type->data[0]);
  *toplevel = true;
  if (*errCode != 0) {
    b_m2c_error(*errCode);
  }
...
void petscMatSetType_initialize(void)
{
}

void petscMatSetType_terminate(void)
{
}
